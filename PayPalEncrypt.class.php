...
class PayPalEncrypt {
     ...
     *
     * @param array $var Associative array of PayPal params. See: https://developer.paypal.com/docs/archive/paypal-payments-standard/integration-guide/Appx-websitestandard-htmlvariables/
     *
     * @return string Encrypted params and encode in Base64
     */
    public function encrypt(array $vars): string {
        ...
        try {
            $file_data = \tempnam(\sys_get_temp_dir(), PayPalEncrypt::TMP_PREFIX);
            $file_sign = \tempnam(\sys_get_temp_dir(), PayPalEncrypt::TMP_PREFIX);
            $file_encr = \tempnam(\sys_get_temp_dir(), PayPalEncrypt::TMP_PREFIX);
             ...
             * See: https://docs.openssl.org/3.0/man3/SMIME_write_PKCS7/#bugs
             */
            if ($this->needsBinaryWorkaround()) {
    ...
    /**
     * Extension point: return false in a subclass once the OpenSSL/PHP
     * SMIME_write_PKCS7 bug is fixed and openssl_pkcs7_sign() with
     * PKCS7_BINARY actually produces raw DER output. Default true
     * preserves the historical workaround behavior.
     */
    protected function needsBinaryWorkaround(): bool {
        return true;
    }

    protected function decodeFileToBinary(string $fileName) {
    ...
    protected function getDataWthoutMime(string $fileName): string {
    ...
    protected function saveBinaryData(string $fileName, $binary) {
    ...
    protected function getDataWithPKCS7Headers(string $fileName): string {
        $data = "-----BEGIN PKCS7-----\n";
        $data .= $this->getDataWthoutMime($fileName);
        $data .= "-----END PKCS7-----\n";
